...
def grow(node, nfeature, Ops, Op_weights, Op_type, alpha1, beta, sigma_a, sigma_b):
    depth = node.depth

    # deciding the number of child nodes
    if node.depth > 0:
        prob = alpha1 / np.power((1 + depth), -beta)
    ...
    if node.type == 0:
        ...
        grow(node.left, nfeature, Ops, Op_weights, Op_type, alpha1, beta, sigma_a, sigma_b)
        ...
        grow(node.left, nfeature, Ops, Op_weights, Op_type, alpha1, beta, sigma_a, sigma_b)
        grow(node.right, nfeature, Ops, Op_weights, Op_type, alpha1, beta, sigma_a, sigma_b)
...
# =============================================================================
def Prop(Root, n_feature, Ops, Op_weights, Op_type, alpha1, beta, sigma_a, sigma_b):
    ###############################
    ######### preparations ########
    ###############################
    ...
    # grow
        ...
        grow(Term[pod], n_feature, Ops, Op_weights, Op_type, alpha1, beta, sigma_a, sigma_b)
    ...
    # transformation
        ...
        if ins_type == 1:  # unary
            ...
            if ins_node.parent is None:  # inserted node is root
                ...
                upDepth(Root)
                grow(new_right, n_feature, Ops, Op_weights, Op_type, alpha1, beta, sigma_a, sigma_b)
    ...
    # reassignOperator
        ...
        if last_type == 1:
            if new_type == 1:  # unary to unary
                ...
                grow(cnode.right, n_feature, Ops, Op_weights, Op_type, alpha1, beta, sigma_a, sigma_b)
...
# =============================================================================
def newProp(Roots, count, sigma, y, indata, n_feature, Ops, Op_weights, Op_type, alpha1, beta, sigma_a, sigma_b):
    ...
    [oldRoot, Root, lnPointers, change, Q, Qinv, last_a, last_b, cnode] = Prop(Root, n_feature, Ops, Op_weights,
                                                                               Op_type, alpha1, beta, sigma_a, sigma_b)
    # print("change:",change)
